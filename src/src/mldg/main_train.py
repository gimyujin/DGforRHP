...
from train import train_one_epoch_mldg_amp
...
def main():
    ...
    exp = cfg.get("exp", {})
    algo = exp.get("algo", "mldg")
    run_id = exp.get("run_id", "v1")

    if algo != "mldg":
        raise ValueError("This entrypoint is for mldg only. exp.algo must be 'mldg'.")
    ...
    mcfg = cfg.get("mldg", {})
    inner_lr = float(mcfg.get("inner_lr", 1e-3))
    lam = float(mcfg.get("lam", 1.0))
    steps_per_epoch = mcfg.get("steps_per_epoch", None)
    ...
    # ----- domain loaders for MLDG train
    domain_loaders = {}
    for dname in sorted(splits["train"][domain_col].unique().tolist()):
        df_d = splits["train"][splits["train"][domain_col] == dname]
        domain_loaders[dname] = build_dataloader(
            df_d, tokenizer,
    ...
    print("MLDG domain loaders:", {k: len(v) for k, v in domain_loaders.items()})
    ...
    if num_cols:
        ...
        model = BertTextNumClassifier(model_name=model_name, num_features=len(num_cols), num_labels=2, dropout=dropout).to(device)
    else:
        from model import BertTextClassifier
        model = BertTextClassifier(model_name=model_name, num_labels=2, dropout=dropout).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        train_acc = float(tr.get("acc_q", 0.0))

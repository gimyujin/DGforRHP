# src/common/eval_unseen_groupdro.py
...
def parse_args():
    ...
    # optional override (in case ckpt didn't store model_name)
...
def main():
    ...
    # model_name priority: CLI override > ckpt > default
    model_name = args.model_name or ckpt.get("model_name") or DEFAULT_MODEL_NAME

    ckpt_num_cols = ckpt.get("num_cols", None)

    # num_cols priority: CLI > ckpt > text-only
    ...
    # NOTE:
    # eval_unseen_groupdro.py 는 src/common/에 두고,
    # 실행은 프로젝트 루트에서: python src/common/eval_unseen_groupdro.py ...
    from model import BertTextClassifier, BertTextNumClassifier
    ...
    out_dir = Path("outputs") / "unseen_eval" / ALGO_NAME / dname
    out_dir.mkdir(parents=True, exist_ok=True)

    out_json = out_dir / f"{dname}_report.json"
    safe_json_dump(rep, out_json)

    print(f"[{ALGO_NAME}/{dname}] n={rep['n']:,} acc={rep['acc']:.4f} f1_macro={rep['f1_macro']:.4f}")

...
def evaluate_by_domain_dann(
    ...
    """
    DANN validation/test score for DG:
      1) 도메인별 metric 계산
      2) 도메인 균등 평균(unweighted mean) = 최종 score

    Assumptions:
      - batch keys: input_ids, attention_mask, labels, domain_id, (optional) num
      - model forward returns an object with .y_logits (shape [B,2] usually)
    """
    model.eval()

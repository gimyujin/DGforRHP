# src/coral/eval_unseen_coral.py
...
def main():
    ...
    out_dir = Path("outputs") / "unseen_eval" / "coral" / dname
    out_dir.mkdir(parents=True, exist_ok=True)

    out_json = out_dir / f"{dname}_report.json"
    safe_json_dump(rep, out_json)

    print(f"[coral/{dname}] n={rep['n']:,} acc={rep['acc']:.4f} f1_macro={rep['f1_macro']:.4f}")
